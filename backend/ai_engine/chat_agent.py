...
def chat(req: ChatRequest) -> ChatResponse:
    ...
    # (no server-side session store) -- the shared MongoDB layer can persist
    # `history` per user and replay it here each call, if needed later.
    convo = ""
    ...
    reply = gemini.ask_text(SYSTEM_PROMPT + context_block, convo, max_tokens=800)
